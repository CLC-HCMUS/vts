...
def summary_is_too_long(selected, documents, lengthUnit, summarySize):
  if lengthUnit == UNIT_CHARACTERS:
    ...
    return len(selected) > summarySize
def get_def_sentsims(documents, stopwords, idfs):
...
def select_sentences(summarySize,
  ...
  if use_aggregate == False:
    ...
    for matrix in matrices:
      ...
      end_checksingleton = time.time()
      print ("clustering {0}, optimizer {1}, check singleton {2}".format(end_clustering - start_clustering,
                                                                        end_submordular - end_clustering,
                                                                        end_checksingleton - end_submordular))
...
#   rng = random.RandomState(4488)
#   w2v_matrix = numpy.zeros((len(flat_sentences), len(flat_sentences)))
#   skipthought_model
#   for i in range(0, len(flat_sentences)):
#     sentence_embedding_i = skipthought_model.encode([])
#     for j in range(i, len(flat_sentences)):
#       sentence_embedding_j = get_sentence_embedding_avg(flat_sentences[j], wordmodel, w2v_backend, quiet=quiet)
#       score = 0.0
#       if not (sentence_embedding_i is None or sentence_embedding_j is None):
...
def summarize_documents(documents,
  ...
  sentsims = get_def_sentsims(documents, stopwords, None)
...
def summarize_files(document_names, length=DEFAULT_SUMMARY_LENGTH, unit=UNIT_WORDS, use_tfidf_similarity=True, use_sentiment_similarity=True, use_w2v_similarity=True, split_sentences=True, w2v_vector_file=W2V_VECTOR_FILE, preloaded_w2v_wordmodel=None, w2v_backend=False, w2v_experiments="", quiet=False, output_numbers=False, use_aggregate_for_clustering=False, anaphora_resolution_simple=False, min_sentence_length=MIN_SENTENCE_LENGTH, input_limit=None):
  ...
  return summarize_documents(documents,
                             length=length,
                             unit=unit,
                             use_tfidf_similarity=use_tfidf_similarity,
                             use_sentiment_similarity=use_sentiment_similarity,
                             use_w2v_similarity=use_w2v_similarity,
                             w2v_vector_file=w2v_vector_file,
                             preloaded_w2v_wordmodel=preloaded_w2v_wordmodel,
                             w2v_backend=w2v_backend,
                             w2v_experiments=w2v_experiments,
                             quiet=quiet,
                             output_numbers=output_numbers,
                             use_aggregate_for_clustering=use_aggregate_for_clustering,
                             anaphora_resolution_simple=anaphora_resolution_simple,
                             min_sentence_length=min_sentence_length)
...
def load_w2v_wordmodel(w2v_vector_file=W2V_VECTOR_FILE):
  ...
  if not os.path.isfile(w2v_vector_file):
    ...
    return word2vec.Word2Vec.load_word2vec_format(w2v_vector_file, binary=False)
...
def hymain():
  ...
  summary_length = 2
  summary_length_unit = 2

  w2v_vector_file = "/Users/HyNguyen/PycharmProjects/summarynew/model/cwvector.txt"
  w2v_backend = False
  w2v_preload = load_w2v_wordmodel(w2v_vector_file)
  w2v_experiments = ""
  ...
  file_path = "accuracy_garmin_nuvi_255W_gps.txt.data"

  sum = summarize_files([opinosis_path+"/"+file_path],
                  length=summary_length,
                  unit=summary_length_unit,
                  use_tfidf_similarity=False,
                  use_sentiment_similarity=False,
                  use_w2v_similarity=True,
                  w2v_backend=False,
                  preloaded_w2v_wordmodel=w2v_preload,quiet=True)
  print (sum)
